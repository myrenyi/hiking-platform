  ...
  return (
    <div className="min-h-screen bg-bg">
        ...
        <div className="grid grid-cols-6 gap-4">
          {safetyCategories.map((cat) => {
            ...
            return (
              <button
                key={cat.id}
                className="bg-surface rounded-xl p-5 border border-surface-200 hover:border-earth transition-all group text-center"
              >
      ...
      <section className="max-w-7xl mx-auto px-6 py-12">
        <h2 className="text-xl font-bold text-forest-deep mb-6">安全知识</h2>
        <div className="grid grid-cols-1 gap-4">
          {displayedTips.map((tip, index) => {
            ...
            return (
              <div
                key={index}
                className="bg-surface rounded-xl border border-surface-200 overflow-hidden"
              >
                <button
                  onClick={() => setExpandedTip(isExpanded ? null : index)}
                  className="w-full grid grid-cols-[1fr_auto] gap-4 items-center p-5 text-left hover:bg-surface-200 transition-colors"
                >
                  <div className="grid grid-cols-[40px_1fr] gap-4 items-center">
                    <Icon size={24} weight="light" strokeWidth={1.5} className="text-forest" />
                    <div>
                      <span className="text-xs text-muted block mb-1">{tip.category}</span>
                      <h3 className="text-base font-bold text-forest-deep">{tip.title}</h3>
                    </div>
                  </div>
                  {isExpanded ? (
                      ...
                      {tip.content.map((line, lineIndex) => (
                        <p
                          ...
                          className={`text-sm leading-relaxed ${
                            line === '' ? 'h-2' : line.endsWith('：') ? 'text-forest-deep font-medium' : 'text-muted'
                          }`}
                        >
                          {line}
                        </p>
        ...
        {safetyTips.length > 2 && (
          <div className="mt-4 text-center">
            ...
            >
              {showAllTips ? '收起' : '展开更多'}
              {showAllTips ? <CaretUp size={16} weight="light" /> : <CaretDown size={16} weight="light" />}
            </button>
          </div>
        )}
      </section>
      ...
      {/* Emergency Contacts */}
      <section className="max-w-7xl mx-auto px-6 py-12">
        <h2 className="text-xl font-bold text-forest-deep mb-6">紧急联络</h2>
        <div className="grid grid-cols-4 gap-4">
          {emergencyContacts.map((contact, index) => (
            <div
              key={index}
              className="bg-surface rounded-xl p-5 border border-surface-200"
            >
              <div className="grid grid-cols-1 gap-3">
                ...
                <div>
                  ...
                  <p className="text-lg font-bold text-earth mt-1">{contact.number}</p>
                  <p className="text-xs text-muted mt-1">{contact.available}</p>
                </div>
                <button className="mt-2 w-full py-2 bg-earth text-white text-sm font-medium rounded-lg hover:bg-earth-light transition-colors">
                  一键拨打
                </button>
              </div>
            </div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-12">
        <h2 className="text-xl font-bold text-forest-deep mb-6">无痕山野</h2>
        <div className="bg-forest-deep rounded-2xl p-8 text-white">
          <div className="grid grid-cols-[1fr_2fr] gap-8 items-center">
            <div className="grid grid-cols-1 gap-4">
              <MapTrifold size={48} weight="light" strokeWidth={1.5} className="text-earth" />
